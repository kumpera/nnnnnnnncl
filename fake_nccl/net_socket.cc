...
ncclResult_t ncclNetSocketConnect(int dev, void* opaqueHandle, void** sendComm) {
  ...
  *sendComm = NULL;
  ...
  comm->dev = dev;
  //HACK HACK
  printf("using fake device during net_socket::ncclNetSocketConnect\n");
...
ncclResult_t ncclNetSocketAccept(void* listenComm, void** recvComm) {
  ...
  *recvComm = NULL;
  ...
  //HACK HACK
  printf("using fake device during net_socket::ncclNetSocketAccept\n");
  rComm->cudaDev = FAKE_CUDA_DEVICE;
